...
@autoreleasepool{
if(CGRectGetWidth(child.bounds)<overlaySize.width*0.8)continue;
...
if([NSStringFromClass(subview.class)isEqualToString:@"CCUIHeaderPocketView"]){
headerPocket=subview;
break;
}
}
if(!headerPocket)return;
CGFloat pocketHeight=CGRectGetHeight(headerPocket.bounds);
...
BOOL bottomHairline=CGRectGetWidth(child.bounds)>CGRectGetWidth(headerPocket.bounds)*0.5&&
...
BOOL bottomHairline=CGRectGetWidth(layer.bounds)>CGRectGetWidth(headerPocket.bounds)*0.5&&
...
dispatch_once(&onceToken,^{lastClampTimes=[NSMutableDictionary dictionary];});
...
static NSMutableSet<NSString*>*gCCAPendingOptimisticCleanups;
...
static dispatch_once_t onceToken;
dispatch_once(&onceToken,^{normalizeDebounces=[NSMutableDictionary dictionary];});
NSString*key=[NSString stringWithFormat:@"%p",overlay];
