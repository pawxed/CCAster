...
static CFTimeInterval gCCALastDisplayLinkTime = 0.0;
static CFTimeInterval gCCAMinDisplayLinkInterval = 1.0 / 30.0;
...
static UIScrollView *CCANativeScrollViewForOverlay(UIViewController *overlay) {
    ...
    static const void *kCCANativeScrollCacheKey = &kCCANativeScrollCacheKey;
    UIScrollView *cached = objc_getAssociatedObject(overlay, kCCANativeScrollCacheKey);
    if (cached) return cached;
    @try {
        id scrollView = [(id)overlay valueForKey:@"overlayScrollView"];
        if ([scrollView isKindOfClass:[UIScrollView class]]) {
            objc_setAssociatedObject(overlay, kCCANativeScrollCacheKey, scrollView, OBJC_ASSOCIATION_ASSIGN);
            return scrollView;
        }
...
static BOOL CCAViewTreeContainsClassNameDepth(UIView *root, NSString *className, NSInteger depth) {
    if (!root || !className.length || depth <= 0) return NO;
    if ([NSStringFromClass(root.class) isEqualToString:className]) return YES;
    for (UIView *subview in root.subviews) {
        if (CCAViewTreeContainsClassNameDepth(subview, className, depth - 1)) return YES;
    }
    return NO;
}
#define CCAViewTreeContainsClassName(root, className) CCAViewTreeContainsClassNameDepth(root, className, 12)
...
static void CCASuppressNativeOverflowMaterialForOverlay(UIViewController *overlay) {
    ...
    static const void *kCCAOverflowSuppressedKey = &kCCAOverflowSuppressedKey;
    if ([objc_getAssociatedObject(overlay, kCCAOverflowSuppressedKey) boolValue]) return;
    objc_setAssociatedObject(overlay, kCCAOverflowSuppressedKey, @YES, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        objc_setAssociatedObject(overlay, kCCAOverflowSuppressedKey, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    });
...
static void CCASuppressHeaderPocketMaterialForOverlay(UIViewController *overlay) {
    ...
    static const void *kCCAPocketSuppressedKey = &kCCAPocketSuppressedKey;
    if ([objc_getAssociatedObject(overlay, kCCAPocketSuppressedKey) boolValue]) return;
    objc_setAssociatedObject(overlay, kCCAPocketSuppressedKey, @YES, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        objc_setAssociatedObject(overlay, kCCAPocketSuppressedKey, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    });
...
static void CCAClampNativeScrollViewForOverlay(UIViewController *overlay) {
    static const void *kCCAClampInProgressKey = &kCCAClampInProgressKey;
    if (!overlay || [objc_getAssociatedObject(overlay, kCCAClampInProgressKey) boolValue]) return;
    objc_setAssociatedObject(overlay, kCCAClampInProgressKey, @YES, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    UIScrollView *scrollView = CCANativeScrollViewForOverlay(overlay);
    if (!scrollView) { objc_setAssociatedObject(overlay, kCCAClampInProgressKey, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC); return; }
    ...
    objc_setAssociatedObject(overlay, kCCAClampInProgressKey, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
static CGFloat CCANativeScrollOpeningCompensation(UIViewController *overlay) {
    if (!gCCAUseNativeOpeningCompensation || !overlay) return 0.0;
    UIScrollView *scrollView = CCANativeScrollViewForOverlay(overlay);
...
static NSArray<UIViewController *> *CCACollectModuleControllers(UIViewController *root) {
    ...
    NSString *key = [NSString stringWithFormat:@"%p_%lu_%u", root, (unsigned long)root.childViewControllers.count, (unsigned)root.view.subviews.count];
...
static UIView *CCAFindSubviewWithClassName(UIView *root, NSString *className, NSInteger depth) {
    if (!root || !className.length || depth <= 0) return nil;
    ...
    for (UIView *subview in subviews) {
        UIView *match = CCAFindSubviewWithClassName(subview, className, depth - 1);
        if (match) return match;
    }
    return nil;
}
#define CCAFindSubviewWithClassName(root, className) CCAFindSubviewWithClassName(root, className, 10)
...
- (void)ownedDuplicateHostDisplayLinkFired:(__unused CADisplayLink *)displayLink {
    ...
    if (now - gCCALastDisplayLinkTime < gCCAMinDisplayLinkInterval) return;
    gCCALastDisplayLinkTime = now;
    ...
    if (gesture.state == UIGestureRecognizerStateEnded || gesture.state == UIGestureRecognizerStateCancelled || gesture.state == UIGestureRecognizerStateFailed) {
        ...
        if (!gEditModeActive) {
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.08 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                ...
                if (fabs(gCCAPagerInteractiveTranslation) < 0.01) [self applyPageTransformToOverlay:overlay animated:NO];
            });
        }
        objc_setAssociatedObject(gesture, kCCAPagerStartPageKey, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (void)resettleAfterCollectionRebuild {
    static BOOL sResettlePending = NO;
    if (sResettlePending) return;
    sResettlePending = YES;
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.3 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        if (!gEditModeActive) return;
        for (UIViewController *candidate in modules) [self applyEditingToModule:candidate editing:YES];
...
- (void)resizePanned:(UIPanGestureRecognizer *)gesture {
    static BOOL sResizePanGuard = NO;
    if (sResizePanGuard) return;
    sResizePanGuard = YES;
...
- (void)modulePanned:(UIGestureRecognizer *)gesture {
    static BOOL sModulePanGuard = NO;
    if (sModulePanGuard) return;
    sModulePanGuard = YES;
    UIViewController *overlay = nil;
    UIResponder *responder = gesture.view;
    while (responder && ![responder isKindOfClass:[UIViewController class]]) responder = responder.nextResponder;
...
- (void)applyEditingToModule:(UIViewController *)module editing:(BOOL)editing {
    ...
    static NSMutableSet *sEditingModules = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{ sEditingModules = [NSMutableSet set]; });
    NSString *guardKey = [NSString stringWithFormat:@"%p_%d", module, (int)editing];
    if ([sEditingModules containsObject:guardKey]) return;
    [sEditingModules addObject:guardKey];
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{ [sEditingModules removeObject:guardKey]; });
    UIView *view = module.view;
    if (!view) return;
